...
```
## Build a predictive pipeline with `tabular_learner` {auto-animate="true"}
```{.python}
import skrub
from sklearn.linear_model import Ridge
...
dim_reduction = X.skb.apply(
    skrub.choose_from(
        {
            "PCA": PCA(skrub.choose_int(10, 30)),
            "SelectKBest": SelectKBest(skrub.choose_int(10, 30))
        }, name="dim_reduction"
    )
)
regressor = dim_reduction.skb.apply(
    skrub.choose_from(
        {
            "Ridge": Ridge(skrub.choose_float(0.1, 10.0, log=True)),
