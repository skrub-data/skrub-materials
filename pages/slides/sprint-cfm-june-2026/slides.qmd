...
## A quick intro to DataOps... {.smaller}
Data Ops:
...
to keep up to date with `upstream/main` to avoid _conflicts_. This can be done by
routinely **merging** your branch with `upstream/main`. If there are conflicts,
they must be resolved before merging.
...
browser (or `open doc/_build/html/index.html`).

## Working on the documentation on Windows

::: {.callout-warning}

On Windows, building the documentation may fail because it requires specific permissions.
In that case, let the CI build the documentation and check it from the PR page. 
